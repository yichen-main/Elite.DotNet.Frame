...
file sealed class SQLiteHelper : ISQLiteHelper
{
    ...
    public Task<IEnumerable<T>> SelectAsync<T>(in SqliteConnection sqlite, in string column, in string id) where T : SQLiteBase =>
        sqlite.QueryAsync<T>(TableLayout.LetSelect<T>(column, id));
    ...
    {
        return new($"Data Source={SQLite($"{FileLayout.GetProjectName(type)}{Extension.Pile}")}");
        static string SQLite(in string name) => System.IO.Path.Combine(FileLayout.GetRootFolderPath(nameof(SQLite)), name);
    }
}