...
public static class ReflexExpand
{
    ...
    {
        foreach (var name in assembly.GetManifestResourceNames().Where(x => x.EndsWith(Extension.Jaon, StringComparison.OrdinalIgnoreCase)))
        {
    ...
    public static string ToJson<T>(this T content, in bool indented = true) => JsonSerializer.Serialize(content, typeof(T), GetJsonOption(indented));
    ...
    public static JsonSerializerOptions GetJsonOption(in bool indented = false) => new()
    {
        MaxDepth = 100,
