...
public static class FeatureExpand
{
    ...
    public static double ParseDouble(this string? value) => double.TryParse(value, CultureInfo.InvariantCulture, out double result) ? result : default;
    static double GetIntegerDigits(this int value) => Convert.ToDouble(TextExpand.EnableTag.PadRight(value + 1, '0'), CultureInfo.InvariantCulture);
    ...
    public static IEnumerable<T> OrEmptyIfNull<T>(this IEnumerable<T>? sources) => sources ?? [];
