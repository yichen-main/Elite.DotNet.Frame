...
public static class ProfileExpand
{
    ...
    public static string RandomEncode256Bits()
    {
    ...
    {
        await foreach (var info in ArchiveExpand.HistoryTransfer.Reader.ReadAllAsync().ConfigureAwait(false))
        {
            var names = info.Type.Name.Split("__");
            var directory = names.Length is 2 ? names[1] : info.Type.Name;
            var title = $"[{ArchiveExpand.Timestamp.ToString("HH:mm:ss", CultureInfo.InvariantCulture)}]";
            ...
            await ArchiveExpand.RecordAsync(directory, $"{title}{label} {content}").ConfigureAwait(false);
