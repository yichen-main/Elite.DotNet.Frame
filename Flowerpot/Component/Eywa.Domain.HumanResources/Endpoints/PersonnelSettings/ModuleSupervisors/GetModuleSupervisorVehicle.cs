...
internal sealed class GetModuleSupervisorVehicle : NodeEnlarge<GetModuleSupervisorVehicle, NodeIdentify>
{
    ...
    public override Task HandleAsync(NodeIdentify req, CancellationToken ct)
    {
        return ExecuteAsync(async (connection, readerAsync) =>
        {
            ...
            var module = await reader.ReadSingleAsync<HumanUserModule>().ConfigureAwait(false);
